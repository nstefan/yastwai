...
├── plans/
│   └── TEMPLATE.md                    <- Plan mode template
├── tests/                             <- Bash test suite
├── .claude/
│   ├── settings.json                  <- Plans directory + hooks config
│   ├── hooks/
│   │   └── compact-context.sh         <- Re-inject git context after compaction
│   ├── agents/
│   │   └── codex-analyst.md           <- Codex Analyst subagent
│   └── commands/
│       ├── cp.md                      <- /cp — commit and push
│       ├── codex.md                   <- /codex — deep code analysis
│       ├── handoff.md                 <- /handoff — session handoff
│       ├── process-doc.md             <- /process-doc — document processing
...
```bash
mkdir -p docs/discovery docs/research docs/requirements docs/strategy docs/summaries docs/archive/handoffs docs/context/client-briefs
mkdir -p output/schemas output/prompts output/deliverables output/presentations
mkdir -p templates plans tests .claude/agents .claude/commands .claude/hooks
```
